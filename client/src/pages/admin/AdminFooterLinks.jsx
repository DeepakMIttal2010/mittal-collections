...
import {
  ...
  updateFooterLink,
  deleteFooterLink,
} from "../../services/adminFooterLinkService";
...
function AdminFooterLinks() {
  ...
  const loadData = async () => {
    ...
    const response = await getAllFooterLinksAdmin({ sortBy, sortOrder });

    if (response.success) setLinks(response.links);
  ...
  const handleSubmit = async (e) => {
    ...
    const response = editingId
      ? await updateFooterLink(editingId, formData)
      : await addFooterLink(formData);
  ...
  const handleDelete = async (id) => {
    ...
    const response = await deleteFooterLink(id);
  ...
  return (
    <div className="p-6">
      ...
      <p className="text-sm text-slate-500 mb-6">
        Manage the links shown under &quot;Company&quot; in the site footer.
