...
export const addTrendingSection = async (req, res) => {
  try {
    ...
    // The unique index on `category` (see TrendingSection.js) is what
    // actually stops two concurrent submissions for the same category
    // from both creating a section — the findOne pre-check above can't
    // fully close that race.
    if (error.code === 11000) {
      return res.status(400).json({
        success: false,
